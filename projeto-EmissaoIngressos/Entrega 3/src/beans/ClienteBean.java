...
public class ClienteBean implements Serializable {
    ...
    public SelectItem[] getIngressos(boolean filtrar) {
        SelectItem[] items;
        int length = Cliente.Ingresso.values().length;
        int n = 0;
        if (filtrar) {
        ...
        for(Cliente.Ingresso ingresso : Cliente.Ingresso.values()) {
            items[n++] = new SelectItem(ingresso, ingresso.getLabel());
        }
        return items;
    }
